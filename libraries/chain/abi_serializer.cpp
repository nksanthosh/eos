...
namespace eosio { namespace chain {
   ...
   namespace impl {
      ...
      void abi_traverse_context_with_path::hint_array_type_if_in_array() {
         if( path.size() == 0 || !fc::holds_alternative<array_index_path_item>(path.back()) )
            return;
      ...
      struct path_item_type_visitor {
         ...
         void operator()( const array_index_path_item& item ) {
            ...
            if( fc::holds_alternative<struct_type_path_root>(th) ) {
               const auto& str = fc::get<struct_type_path_root>(th).struct_itr->first;
               output_name( s, str, shorten_names );
            } else if( fc::holds_alternative<variant_type_path_root>(th) ) {
               const auto& str = fc::get<variant_type_path_root>(th).variant_itr->first;
               output_name( s, str, shorten_names );
            } else if( fc::holds_alternative<array_type_path_root>(th) ) {
               s << "ARRAY";
            } else {
               s << "UNKNOWN";
            }
      ...
      string abi_traverse_context_with_path::get_path_string()const {
         ...
         for( size_t i = 0, n = path.size(); i < n; ++i ) {
            if( full_path && !fc::holds_alternative<array_index_path_item>(path[i]) )
               visitor.add_dot();
         ...
         if( !full_path ) {
            if( fc::holds_alternative<empty_path_item>(visitor.last_path_item) ) {
               fc::visit( visitor, root_of_path );
            } else {
