...
int apply_context::get_context_free_data( uint32_t index, char* buffer, size_t buffer_size )const
{
   ...
   if( fc::holds_alternative<packed_transaction::prunable_data_type::none>(data) ) {
   } else if( fc::holds_alternative<packed_transaction::prunable_data_type::partial>(data) ) {
      if( index >= fc::get<packed_transaction::prunable_data_type::partial>(data).context_free_segments.size() ) return -1;

      cfd = trx_context.packed_trx.get_context_free_data(index);
   } else {
      const std::vector<bytes>& context_free_data =
            fc::holds_alternative<packed_transaction::prunable_data_type::full_legacy>(data) ?
               fc::get<packed_transaction::prunable_data_type::full_legacy>(data).context_free_segments :
